...
function refresh_plots() {
  d3.csv("http://vis.lab.djosix.com:2020/data/spotify_tracks.csv").then( function(data) {
    ...
    for (let i=0; i<instrumentalness_top[0].length; i++) {
      if (isNaN(instrumentalness_all[0][i]) || isNaN(instrumentalness_top[0][i])) {continue;}
    ...
    var layout = {
      showlegend: true, 
      title: "Characteristic Relevance for Top ".concat(top_n_tracks, " Tracks")
      }
    ...
    energy_list.forEach(() => (all_labels.push("Energy")));
    danceability_list.forEach(() => (all_labels.push("Danceability")));
    speechiness_list.forEach(() => (all_labels.push("Speechiness")));
    acousticness_list.forEach(() => (all_labels.push("Acousticness")));
    instrumentalness_list.forEach(() => (all_labels.push("Instrumentalness")));
    liveness_list.forEach(() => (all_labels.push("Liveness")));

    all_data = energy_list.concat(danceability_list, speechiness_list, acousticness_list, instrumentalness_list, liveness_list);


    top_labels = new Array();
    energy_list.slice(0,top_n_tracks).forEach(() => (top_labels.push("Energy")));
    danceability_list.slice(0,top_n_tracks).forEach(() => (top_labels.push("Danceability")));
    speechiness_list.slice(0,top_n_tracks).forEach(() => (top_labels.push("Speechiness")));
    acousticness_list.slice(0,top_n_tracks).forEach(() => (top_labels.push("Acousticness")));
    instrumentalness_list.slice(0,top_n_tracks).forEach(() => (top_labels.push("Instrumentalness")));
    liveness_list.slice(0,top_n_tracks).forEach(() => (top_labels.push("Liveness")));

    top_data = energy_list.slice(0,top_n_tracks).concat(danceability_list.slice(0,top_n_tracks), speechiness_list.slice(0,top_n_tracks), acousticness_list.slice(0,top_n_tracks), instrumentalness_list.slice(0,top_n_tracks), liveness_list.slice(0,top_n_tracks));

    var trace1 = {
      y: all_data,
      x: all_labels,
      name: 'All Songs',
      marker: {color: 'rgb(201, 115, 2)'},
      type: 'box', 
      boxpoints: false
    };
    ...
    var layout = {
      ...
      boxmode: 'group',
      title: "Individual Feature Statistical Overview"
    };

    Plotly.newPlot('feature_div', data, layout);
